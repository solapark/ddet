...
save_dir = '/data3/sap/VEDet/result/tmvreid_messytable_asnet/4'
bands, max_freq = 64, 8
num_views = 9
num_input = 76
...
model = dict(
    ...
    #        'stage5',
    #    )),
    #img_neck=dict(type='CPFPN', in_channels=[1024], out_channels=512, num_outs=1),
        ...
        det_transformer=dict(
            ...
            det_decoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(type='MultiheadAttention', embed_dims=1024, num_heads=8, dropout=0.1),
                        dict(type='PETRMultiheadAttention', embed_dims=1024, num_heads=8, dropout=0.1),
                    ],
...
total_epochs = 200
gt_path = '/data1/sap/MessyTable/labels/val.json'
#gt_path = '/data1/sap/MessyTable/labels/test.json'
evaluation = dict(interval=20, pipeline=test_pipeline, metric=['bbox'], show=False, eval_thresh=.1, visible_thresh=.5, reid_thresh=.1, save_dir=save_dir, img_root='/data1/sap/MessyTable/images/', gt_path=gt_path)
