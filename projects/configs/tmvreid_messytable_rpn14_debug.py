...
save_dir = '/data3/sap/VEDet/result/tmvreid_messytable_rpn/14_ep200'
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        output_det_encoding=dict(
            type='FourierMLPEncoding',
            input_channels=16,
            hidden_dims=[int(1.5 * 16 * 2 * bands)],
            embed_dim=256,
            fourier_type='linear',
            fourier_channels=16 * 2 * bands,
            max_frequency=max_freq),
        ...
        loss_self_attn_map=dict(type='FocalLoss', use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=2.0/10),
    ),
...
total_epochs = 200
evaluation = dict(interval=10, pipeline=test_pipeline, metric=['bbox'], show=True, eval_thresh=.1, visible_thresh=.5, reid_thresh=.1, save_dir=save_dir, img_root='/data1/sap/MessyTable/images/', show_query=True)
...
load_from = 'work_dirs/tmvreid_messytable_rpn14/epoch_200.pth'
resume_from = None
