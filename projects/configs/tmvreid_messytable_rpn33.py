...
save_dir = '/home/sapark/VEDet/result/tmvreid_messytable_rpn33'
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        det_transformer=dict(
            ...
            attn_mask_type = ['cross_attn', 'self_attn'],
            det_decoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        ...
                        dict(type='TMVReidMultiheadCrossAttention', embed_dims=256, num_heads=8, dropout=0.1,
                            #attention=dict(type='MultiheadSuperAttention', other_view_wgt=1., num_views=num_views, num_query=num_query, num_key=num_rpn_per_view, top_simK=2)
                        ),
                        dict(type='TMVReidMultiheadSelfAttention', embed_dims=127, num_heads=1, dropout=0.1),
                    ],
                    feedforward_channels=2048,
                    ffn_dropout=0.1,
                    with_cp=True,
                    operation_order=('cross_attn', 'self_attn', 'norm', 'ffn', 'norm'),
                    attn_to_next_layer = [True, False],
                    ),
            )),
        bbox_coder=dict(
            #type='NMSFreeCoder',
        ...
        loss_self_attn_map=dict(type='FocalLoss', use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=2.0),
    ),
