...
class QueryCost(object):
    ...
    def __call__(self, cxcy_pred, gt_cxcy, gt_visible):
        """
        Args:
            cxcy_pred (Tensor): Predicted boxes with normalized coordinates
                (cx, cy), which are all in range [0, 1]. Shape
                [num_query, (num_views)*2].
            gt_cxcy (Tensor): Ground truth boxes with normalized coordinates
                (cx, cy), which are all in range [0, 1]. Shape
        ...
        """
        num_pred = len(cxcy_pred)
        num_gt = len(gt_cxcy)
        cxcy_pred = cxcy_pred.reshape(num_pred, self.num_views, 2) #(900, 3, 2)
        gt_cxcy = gt_cxcy.reshape(num_gt, self.num_views, 2) #(num_inst, 3, 2)

        cxcy_cost_list = [] #(num_gt, num_pred, 1)
        for i in range(num_gt):
            #cur_gt_visible = gt_visible[i] #(2, )
            visible_mask = gt_visible[i].to(torch.bool) #(2, )
            cur_gt_cxcys = gt_cxcy[i][visible_mask].reshape(1, -1) #(1, num_valid_views*2)
            cur_cxcy_pred = cxcy_pred[:, visible_mask].reshape(num_pred, -1) #(900, num_valid_views*2)

            cxcy_cost = torch.cdist(cur_cxcy_pred, cur_gt_cxcys, p=1).squeeze(-1) #(900, )
