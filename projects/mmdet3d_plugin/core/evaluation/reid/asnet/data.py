...
class MessyTableDatasetFeatures_homography(Dataset):
    """
    Dataset for inference, stage I: retrieving features
    """
    ...
    def __getitem__(self, idx):
        ...
        for cam, cam_value in content['cameras'].items():
            ...
            info['extrinsics'][cam] = torch.tensor(cam_value['extrinsics'])
            info['intrinsics'][cam] = torch.tensor(np.array(self.intrinsics[cam]).reshape(3, 3))
            info['corners'][cam] = torch.tensor(np.array(cam_value['corners']).reshape(-1, 1, 2))
...
class MessyTableDatasetCompare_frcnn(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for main_inst_id, main_gt_id, main_subcls, main_prob in zip(main_cam_inst_id, main_cam_gt_id, main_cam_subcls, main_cam_prob):
            #for sec_inst_id, sec_pred_id, sec_subcls in zip(sec_cam_inst_id, sec_cam_pred_id, sec_cam_subcls):
            for sec_inst_id, sec_gt_id, sec_subcls in zip(sec_cam_inst_id, sec_cam_gt_id, sec_cam_subcls):
...
class MessyTableDatasetCompare(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return info
class MessyTableDatasetCompare_homography(Dataset):
    """
    Dataset for inference, stage II: compute app and esc distance 
    """
    ...
    def __getitem__(self, idx):
        ...
        main_corners = np.array(self.feat_vec_dict[scene_name][main_cam]['corners'][0])
        sec_corners = np.array(self.feat_vec_dict[scene_name][sec_cam]['corners'][0])
        ...
        homo_distance = homography_constraint(main_bbox_pos, sec_bbox_pos, main_corners, sec_corners, self.config)
        homo_distance = homo_distance.reshape((-1, 1)).astype(np.float32)
        ...
        info = {
            ...
            'gt_subcls': torch.from_numpy(np.array(gt_subcls_list).astype(np.float32)).reshape((-1, 1)),
            #'epi_distance': torch.from_numpy(epi_distance),
            'homo_distance': torch.from_numpy(homo_distance),
            #'angle_difference': torch.from_numpy(np.ones(epi_distance.shape) * angle_difference),
            'shape': np.array([len(main_cam_inst_id), len(sec_cam_inst_id)]),
            'inst_pair': np.array(temp_list_inst_pairs),
            'scene_name': scene_name,
