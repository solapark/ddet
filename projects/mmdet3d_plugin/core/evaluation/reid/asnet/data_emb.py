...
def prepare_training_samples(config):
    ...
    for i in range(batch_num):
        batch_image_pairs = image_pairs_list[
                            ...
                            i * image_pairs_per_batch:(i + 1) * image_pairs_per_batch]
        n_crop_id_subcls = []
        n_crop_id_cls = []
        n_crop_id_others = []
...
class MessyTableDatasetFeatures_frcnn(Dataset):
    """
    ...
    def __getitem__(self, idx):
        ...
        for cam, cam_value in content['cameras'].items():
            ...
            for inst_id, inst_value in cam_value['instances'].items():
                #***
                #temp_attr_list.append([int(cam), int(inst_id), inst_value['subcls'], int(inst_value['pred_id']), float(inst_value['prob'])])  # added subcls
                #temp_attr_list.append([int(cam), int(inst_id), inst_value['subcls'], int(inst_value['gt_id']), float(inst_value['prob'])])  # added subcls
                temp_attr_list.append([int(cam), int(inst_id), inst_value['subcls'], int(inst_value['gt_id']), float(inst_value['prob']), float(inst_value['is_valid']), float(inst_value['emb_dist'])])  # added subcls
...
class MessyTableDatasetCompare_frcnn(Dataset):
    """
    ...
    def __getitem__(self, idx):
        ...
        #main_cam_pred_id = self.feat_vec_dict[scene_name][main_cam]['pred_id']
        main_cam_gt_id = self.feat_vec_dict[scene_name][main_cam]['gt_id']
        sec_cam_inst_id = self.feat_vec_dict[scene_name][sec_cam]['inst_id']
        #sec_cam_pred_id = self.feat_vec_dict[scene_name][sec_cam]['pred_id']
        sec_cam_gt_id = self.feat_vec_dict[scene_name][sec_cam]['gt_id']
        main_cam_subcls = self.feat_vec_dict[scene_name][main_cam]['subcls']
        sec_cam_subcls = self.feat_vec_dict[scene_name][sec_cam]['subcls']
        main_cam_prob = self.feat_vec_dict[scene_name][main_cam]['prob']

        main_cam_is_valid = self.feat_vec_dict[scene_name][main_cam]['is_valid']
        sec_cam_is_valid = self.feat_vec_dict[scene_name][sec_cam]['is_valid']
        main_cam_emb_dist = self.feat_vec_dict[scene_name][main_cam]['emb_dist']
        sec_cam_emb_dist = self.feat_vec_dict[scene_name][sec_cam]['emb_dist']

        gt_inst_list = []
        dist_list = []
        gt_subcls_list = []
        temp_list_inst_pairs = []  # bbox pos
        pred_list_inst_pairs = []  
        cnt = 0
        #for main_inst_id, main_pred_id, main_subcls, main_prob in zip(main_cam_inst_id, main_cam_pred_id, main_cam_subcls, main_cam_prob):
        for main_inst_id, main_gt_id, main_subcls, main_prob, main_is_valid, main_emb_dist in zip(main_cam_inst_id, main_cam_gt_id, main_cam_subcls, main_cam_prob, main_cam_is_valid, main_cam_emb_dist):

            #for sec_inst_id, sec_pred_id, sec_subcls in zip(sec_cam_inst_id, sec_cam_pred_id, sec_cam_subcls):
            for sec_inst_id, sec_gt_id, sec_subcls, sec_is_valid, sec_emb_dist in zip(sec_cam_inst_id, sec_cam_gt_id, sec_cam_subcls, sec_cam_is_valid, sec_cam_emb_dist):
                if main_inst_id != sec_inst_id :
                    continue

                if sec_is_valid == '1' and  main_is_valid == '2' :
                    dist = main_emb_dist

                elif sec_is_valid == '2' and  main_is_valid == '1' :
                    dist = sec_emb_dist
                ...
                gt = 1 if main_gt_id == sec_gt_id else 0
                # ***
                #dist = (1 - float(main_prob)) if main_pred_id == sec_pred_id else 1
                #dist = (1 - float(main_prob)) if main_gt_id == sec_gt_id else 1
                #dist = (1 - float(main_prob)) if main_inst_id == sec_inst_id else 1
                ...
                #pred_list_inst_pairs.append([int(main_pred_id), int(sec_pred_id)])
                #pred_list_inst_pairs.append([int(main_gt_id), int(sec_gt_id)])

        #for a, b, c, d in zip(temp_list_inst_pairs, pred_list_inst_pairs, gt_inst_list, dist_list): print(a, b, c, d)
        ...
        info = {
            ...
            'dist': torch.from_numpy(np.array(dist_list).astype(np.float32)).reshape((-1, 1)),
