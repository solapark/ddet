...
def homography_constraint(
        bbox_list1, bbox_list2, corners1, corners2, config):
    """
    inputs:
    bbox list []
    corners : (N, 1, 2) numpy
    ...
    for i in range(len(bbox_list1)):
        for j in range(len(bbox_list2)):
            ...
            # bbox 1 in camera 2
            bbox1_3dpt = np.array([*bbox1_2dpt, 1])
            bbox1_3dpt = bbox1_3dpt.reshape((3, -1))

            bbox1_in2_3dpt = np.matmul(H, bbox1_3dpt)
            bbox1_in2_3dpt = bbox1_in2_3dpt / bbox1_in2_3dpt[2, :]
            bbox1_in2_2dpt = bbox1_in2_3dpt[:2, :] #(2, N)
            bbox1_in2_2dpt = np.round(bbox1_in2_2dpt, 0).astype(np.int)

            #print(bbox1_2dpt, bbox1_in2_2dpt)
            dist = find_dist(bbox1_in2_2dpt, bbox2_2dpt)
            # measure distance
