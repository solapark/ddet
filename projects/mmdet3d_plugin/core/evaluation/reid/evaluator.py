...
from .asnet.data import *
from .asnet.model import *
from .asnet.utils import *
from .asnet.config import parse_config
...
class Evaluator():
    def __init__(self, config_dir, eval_json):
        self.args = dummy()

        self.args.config_dir = config_dir
        self.args.eval_json = eval_json
        self.args.eval_frcnn = True

        self.args.load_features = False
        self.args.save_features = False
        self.args.eval_model = False
        self.args.eval_model_esc = False
        self.args.eval_by_angle = False
        self.args.eval_homography = False

    def retrieve_features_frcnn(self, feature_loader, config):
        """
    ...
    def retrieve_features(self, model_app, model_neighbor, feature_loader, config):
        """
        retrieve features for measuring distance, reducing from O(n^2) to O(n) during inference;
        this function enables versatile potential usages, including computing both appearnace and 
        neighboring models at the same time; the neighboring model also enables options of 
    ...
    def compute_IPAA_metric(self, results_to_save, app_dist_np, single_or_fusion):
        # init empty dict
        IPAA_dict = {}
        ...
        return IPAA_pct_dict

    def retrieve_features_homography(self, model_app, model_neighbor, feature_loader, config):
        """
        retrieve features for measuring distance, reducing from O(n^2) to O(n) during inference;
        this function enables versatile potential usages, including computing both appearnace and 
        neighboring models at the same time; the neighboring model also enables options of 
        ...
        with torch.no_grad():
            ...
            for sample_dict in feature_loader:
                ...
                feature_vector_dict[scene_name]['extrinsics'] = sample_dict['extrinsics']
                feature_vector_dict[scene_name]['intrinsics'] = sample_dict['intrinsics']
                feature_vector_dict[scene_name]['corners'] = sample_dict['corners']
                idx += 1
                progbar.update(idx)
        ...
        for scene_name, scene_content in feature_vector_dict.items():
            ...
            for cam in cam_list:
                feature_vector_cam_dict[scene_name][cam] = {
                    ...
                    'extrinsics': list(feature_vector_dict[scene_name]['extrinsics'][cam].numpy()),
                    'intrinsics': list(feature_vector_dict[scene_name]['intrinsics'][cam].numpy()),
                    'corners': list(feature_vector_dict[scene_name]['corners'][cam].numpy()),
                    'app_features': {},  # of list
                    'multi_crop_feature': {},  # of list
                }
    ...
    def eval_model(self, config, turn_on_save=True):
        ...
        if not config['load_features']:
            ...
            if zoomout_crop_num == 'single_crop':
                feature_vector_cam_dict, inference_samples = self.retrieve_features(model, None, feature_loader, config)
            if zoomout_crop_num == 'mulitiple_zoomout':
                feature_vector_cam_dict, inference_samples = self.retrieve_features(None, model, feature_loader, config)
        ...
        #self.compute_IPAA_metric(results_to_save, dist_np, 'model_only')
    ...
    def eval_homography(self, config, turn_on_save=True):
        device = config['device']
        dev_label_pathname = config['eval_pathname']
        if not config['load_features']:
            print('retrieving features...')
            ...
            feature_set = MessyTableDatasetFeatures_homography(config, dev_label_pathname)
            feature_loader = DataLoader(feature_set, batch_size=1, shuffle=False, num_workers=config['num_workers'])
            ...
            #model.to(device)
            if zoomout_crop_num == 'single_crop':
                feature_vector_cam_dict, inference_samples = retrieve_features_homography(model, None, feature_loader, config)
            if zoomout_crop_num == 'mulitiple_zoomout':
                feature_vector_cam_dict, inference_samples = retrieve_features_homography(None, model, feature_loader, config)
            compare_set = MessyTableDatasetCompare_homography(inference_samples, feature_vector_cam_dict, config)
            compare_loader = DataLoader(compare_set, batch_size=1, shuffle=False, num_workers=0)
            results_to_save = {}
            with torch.no_grad():
                gt_inst_list = []
                ...
                for sample_dict in compare_loader:
                    ...
                    gt_subcls = sample_dict['gt_subcls'][0]
                    #epi_distance = sample_dict['epi_distance'][0]
                    homo_distance = sample_dict['homo_distance'][0]
                    #angle_difference = sample_dict['angle_difference'][0]
                    ...
                    results_to_save[img_pair] = {
                        ...
                        #'angle_diff': [str(i) for i in list(angle_difference.cpu().detach().numpy().squeeze())],
                        #'app_dist': [str(i) for i in list(dist.cpu().detach().numpy().squeeze())],
                        'homo_dist': [str(i) for i in list(homo_distance.cpu().detach().numpy().squeeze())],
                        'main_bbox_id': [str(i) for i in
                                         list(sample_dict['main_bbox_id'][0].cpu().detach().numpy().squeeze())],
                        'sec_bbox_id': [str(i) for i in
                                        list(sample_dict['sec_bbox_id'][0].cpu().detach().numpy().squeeze())]
                    ...
                    #epi_dist_list += epi_distance.cpu().detach().numpy().squeeze().tolist()
                    #angle_diff_list += angle_difference.cpu().detach().numpy().squeeze().tolist()
                    homo_dist_list += homo_distance.cpu().detach().numpy().squeeze().tolist()
                    idx += 1
                    progbar.update(idx)

                #dist_np = np.array(dist_list).reshape(-1, 1)
                dist_np = np.array(homo_dist_list).reshape(-1, 1)
                gt_inst_np = np.array(gt_inst_list).reshape(-1, 1)
    ...
    def eval_model_esc(self, config):
        results_img_pairs_pathanme = os.path.join(config['config_dir'], 'results_img_pairs.json')
        app_dist_pathname = os.path.join(config['config_dir'], 'app_dist.npy')
        epi_dist_pathname = os.path.join(config['config_dir'], 'epi_dist.npy')
        gt_inst_np_pathname = os.path.join(config['config_dir'], 'gt_inst_np.npy')
        ...
        print('FPR = {:0.3f}'.format(fpr))
        #self.compute_IPAA_metric(results_to_save, overall_dist_np, 'model_esc')


    def eval_by_angle(self, config, mode='model_only'):
        """
    ...
    def eval_frcnn(self, config, turn_on_save=True):
        device = config['device']
        zoomout_crop_num = config['zoomout_crop_num']
        dev_label_pathname = config['eval_pathname']
        if not config['load_features']:
            ...
            feature_vector_cam_dict, inference_samples = self.retrieve_features_frcnn(feature_loader, config)
        ...
        print('AP = {:0.3f}'.format(inst_AP))
        print('FPR = {:0.3f}'.format(fpr))
        #IPAA = self.compute_IPAA_metric(results_to_save, dist_np, 'model_only')
            ...
            #np.save(angle_diff_pathname, angle_diff_np)
            #np.save(gt_inst_np_pathname, gt_inst_np)
        res = dict({'AP':inst_AP, 'fpr':fpr})
        #res.update(IPAA)
        return res

    def main(self):
        print('Configuration file in', self.args.config_dir)
        ...
        config['eval_json'] = self.args.eval_json
        config['eval_pathname'] = os.path.join(config['data_dir'], 'labels', config['eval_json'])
        config['load_features'] = self.args.load_features
        config['save_features'] = self.args.save_features
        config['eval_model'] = self.args.eval_model
        config['eval_model_esc'] = self.args.eval_model_esc
        config['eval_by_angle'] = self.args.eval_by_angle
        config['eval_frcnn'] = self.args.eval_frcnn
        config['eval_homography'] = self.args.eval_homography

        tic = time.time()
        if config['eval_model']:
            print('----evaluating feature model...')
            self.eval_model(config)
        if config['eval_model_esc']:
            print('----evaluating feature model with epipolar soft constraint...')
            self.eval_model_esc(config)
        if config['eval_by_angle']:
            print('----evaluating by angle differences...')
            self.eval_by_angle(config)
        if config['eval_frcnn']:
            print('----evaluating frcnn...')
            res = self.eval_frcnn(config)
        if config['eval_homography']:
            print('----evaluating homography...')
            self.val_homography(config)
        toc = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_homography', action='store_true',
                        help='option to eval homography')
