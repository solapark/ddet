...
from ..utils.bipartite_graph import Bipartite_graph
from ..utils.json_maker import json_maker

import argparse 

class GT_ID_ASSIGNER :
    def __init__(self, gt_path, json_path, num_cam):
        self.gt_path = gt_path
        self.src_path = json_path
        self.dst_path = json_path
        self.test_emb = False 
        self.num_cam = num_cam

    def add_alias_matching(self, matching, pred_insts) :
        alias = dict()
        for pred_inst in pred_insts :
    ...
    def main(self):
        src_json = json_maker([], self.src_path, 0)
        gt_json = json_maker([], self.gt_path, 0)
        dst_json = json_maker([], self.dst_path, 0)
        ...
        for scene_id in tqdm(scene_ids) :
            ...
            for cam_id in cam_ids :
                if int(cam_id) > self.num_cam : break
                G = Bipartite_graph()
                ...
                for pred_inst in pred_insts :
                    for gt_inst in gt_insts :
                        ...
                        wgt = utils.iou(pred_inst['pos'], gt_inst['pos'])
                ...
                matching = self.add_alias_matching(matching, pred_insts)
...
if __name__ == '__main__' :
    ...
    _gt_id_assigner = GT_ID_ASSIGNER(args)
    _gt_id_assigner.main()
