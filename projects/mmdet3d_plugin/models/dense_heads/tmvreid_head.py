...
class TMVReidHead(TMVDetHead):
    ...
    def __init__(self,
        ...
        if self.loss_idx is not None and self.loss_idx.use_sigmoid:
            self.idx_out_channels = 1
        else:
            self.idx_out_channels = 2
        ...
        self.feat_idx_emb = nn.Parameter(torch.rand((H, W, feat_idx_emb_size))) #(300, 1, 3)
        self.register_parameter('feat_idx_emb', feat_idx_emb)
        ...
        reid_branch = []
        for lyr, (in_channel, out_channel) in enumerate(
                zip([self.output_det_encoding.embed_dim] + reid_hidden_dims,
        ...
        idx_branch = []
        for lyr, (in_channel, out_channel) in enumerate(
                zip([self.output_det_encoding.embed_dim] + idx_hidden_dims,
    ...
    def forward(self, inputs, img_metas, is_test=False):
        ...
        pred_box, pred_feats, pred_box_prob = inputs #(1, 3, 300, 1, 4), (1, 3, 300, 1, 128), (1, 3, 300, 1, 1), 
        batch_size, num_cams, input_img_h, input_img_w, _ = pred_feats.shape #1, 3, 300, 1
        masks = pred_feats.new_zeros((batch_size, num_cams, input_img_h, input_img_w)).to(torch.bool) #(1, 3, 300, 1)

        if self.input_ray_encoding is not None:
            pos_embeds = self.position_embedding(pred_box, img_metas) #(1, 3, 300, 1, 132)
        rpn_idx_emb = self.feat_idx_emb.repeat(batch_size, num_cams, 1, 1, 1) #(1, 3, 300, 1, 3)
        feats = torch.stack([pred_feats, pred_prob, rpn_idx_emb], -1),  #(1, 3, 300, 1, 128+1+3)
        feats = feats.flatten(2, 3) #(1, 3, 300*1, 132)
        ...
        if self.det_transformer is not None:
            ...
            self.query3d_norm, self.query3d_denorm = init_det_points, query3d_denorm
        ...
        outs = {
            ...
            'all_reid_scores': reid_scores,
            'all_idx_scores': idx_scores,
    ...
    def _get_target_single(self, cls_score, visible_score, reid_score, idx_score, bbox_pred, gt_labels, gt_visibles, gt_idx, gt_proj_cxcy, gt_bboxes, gt_bboxes_ignore=None):
        ...
        assign_result = self.assigner.assign(bbox_pred, cls_score, visible_score, reid_score, idx_score, self.query2d_norm, gt_labels, gt_visibles, gt_idx, gt_proj_cxcy, gt_bboxes_ignore,
        ...
        return (labels, label_weights, visible_targets, visible_weights, reid_targets, reid_weights, idx_targets, idx_wieghts, bbox_targets, bbox_weights, pos_inds, neg_inds)
    ...
    def get_targets(self, cls_scores_list, visible_scores_list, reid_scores_list, idx_scores_list, bbox_preds_list, gt_bboxes_list, gt_labels_list, gt_visibles_list, gt_idx_list, gt_proj_cxcy_list, gt_bboxes_ignore_list=None):
        ...
        (labels_list, label_weights_list, visibles_list, visible_weights_list, reid_list, reid_weights_list, idx_list, idx_weights_list, bbox_targets_list, bbox_weights_list, pos_inds_list,
         neg_inds_list) = multi_apply(self._get_target_single, cls_scores_list, visible_scores_list, reid_scores_list, idx_scores_list, bbox_preds_list, gt_labels_list, gt_visibles_list, gt_idx_list, gt_bboxes_proj_cxcy, 
    ...
    def loss(
        ...
        if self.loss_cls is not None:
            ...
            losses_cls, losses_visible, losses_reid, losses_idx, losses_bbox, losses_seg = multi_apply(self.loss_single, all_cls_scores, all_visible_scores, all_reid_scores, all_idx_scores, all_bbox_preds,
                                                              all_seg_preds, all_gt_bboxes_list, all_gt_labels_list, all_gt_visibles_list, all_gt_idx_list, all_gt_proj_cxcy_list
    ...
    def get_mtv_points_img(self, init_det_points_mtv, img_metas):
        ...
        init_det_points_mtv_norm = init_det_points_mtv_denorm / pad_shape
    ...
    def position_embedding(self, pred_box, img_metas, mask, depth_maps=None, use_cache_depth=False):
        ...
        geometry = torch.cat([pred_box, ctrs, ], dim=-1) #(1, 3, 300, 1, 4+11+)
